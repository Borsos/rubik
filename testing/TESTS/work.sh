...
test_prex "-e 'cb.random_cube(\"${X}x${Y}x${Z}\")' -o im_${X}x${Y}x${Z}.raw"
check_file_exists_and_has_size im_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

test_prex "-e 'cb.random_cube(\"${X}x${Y}x${Z}\")' -o rtmp_{shape}.{format}"
...
test_prex "-e 'cb.const_cube(\"${X}x${Y}x${Z}\", value=$CONST_VALUE)' -o c_{shape}.{format}"
check_file_exists_and_has_size c_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

test_prex "-i c_{shape}.{format} -s ${X}x${Y}x${Z} -e 'np.sum(i0) - ( $X * $Y * $Z * $CONST_VALUE )' -P > c.out"
check_file_exists_and_has_content c.out 0.0

test_prex "-e 'cb.linear_cube(\"${X}x${Y}x${Z}\", start=2.0)' -o l_{shape}.{format}"
check_file_exists_and_has_size l_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

test_prex "-e 'np.array([[[k for z in range(${Z})] for j in range(${Y})] for k in range(${X})])' -o cp_{shape}.{format}"
check_file_exists_and_has_size cp_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

test_prex "-e 'cb.random_cube(\"${X}x${Y}x${H}x${Z}\")' -o og_{shape}.{format}"
check_file_exists_and_has_size og_${X}x${Y}x${H}x${Z}.raw $(( $XYHZ * ${bytes_float32} ))

test_prex "-i og_{shape}.{format} -s 10x20x501x30 -x :x:x250x: -o og_h250_{shape}.{format}"
check_file_exists_and_has_size og_h250_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

Hincr=10
Hnum=11
Hstop=$(( $Hstart + ( $Hincr * $Hnum ) + 1 ))
test_prex "-i og_{shape}.{format} -s ${X}x${Y}x${H}x${Z} -x :x:x${Hstart}:${Hstop}:${Hincr}x: -o og_h{d2}_{shape}.{format} --split 2"
typeset -i i=0
while [[ $i -lt $Hnum ]] ; do
...
test_prex "-e 'cb.const_blocks_cube(\"10x20x30\")' -o cp_{shape}.{format}"
check_file_exists_and_has_size cp_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

## --- 2D ---
typeset -i _x=8
typeset -i _y=10
test_prex "-e 'cb.linear_cube(\"$_x x $_y\")' -o l1_{shape}.{format}"
check_file_exists_and_has_size l1_${_x}x${_y}.raw $(( $_x * $_y * ${bytes_float32} ))

test_prex "-e 'cb.linear_cube(\"$_x x $_y\", start=1.0, increment=2.0)' -o l2_{shape}.{format}"
check_file_exists_and_has_size l2_${_x}x${_y}.raw $(( $_x * $_y * ${bytes_float32} ))

test_prex "-i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1)' -P > res.out"
check_file_exists_and_has_content res.out 0

test_prex "-i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1, tolerance=1.0)' -P > res.out"
check_file_exists_and_has_content res.out 1

test_prex "-i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1, tolerance=10.0)' -P > res.out"
check_file_exists_and_has_content res.out 10

test_prex "-i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1, tolerance=1e3)' -P > res.out"
