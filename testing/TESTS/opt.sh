...
test_prex "rubik -e 'cb.random_cube(\"${X}x${Y}x${Z}\")' -o x_${X}x${Y}x${Z}.raw"
...
for optimized_min_size in '0' '10b' '100gb' ; do
    for extractor in ':x:x:' '::2x::3x::2' '3:-3x10:-10x20:' ':x:x::2' '1x:x:' ':x1x:' ':x:x1' '::2x1x10:' ; do
        ...
        test_prex "rubik -i x_{shape}.{format} -s $shape --optimized-min-size $optimized_min_size -x $extractor -o xd.raw"
        check_file_exists xd.raw
        test_prex "rubik -i x_{shape}.{format} -s $shape --optimized-min-size $optimized_min_size -x $extractor -o xo.raw --optimized"
        check_file_exists xo.raw
        check_files_are_equal xd.raw xo.raw
        test_prex "rubik -i x_{shape}.{format} -s $shape --optimized-min-size $optimized_min_size -x $extractor -o xs.raw --safe"
