...
test_prex "cubist -e 'cb.random_cube(\"${X}x${Y}x${Z}\")' -o im_${X}x${Y}x${Z}.raw"
check_file_exists_and_has_size im_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

test_prex "cubist -e 'cb.random_cube(\"${X}x${Y}x${Z}\")' -o rtmp_{shape}.{format}"
...
test_prex "cubist -e 'cb.const_cube(\"${X}x${Y}x${Z}\", value=$CONST_VALUE)' -o c_{shape}.{format}"
...
test_prex "cubist -e 'cb.linear_cube(\"${X}x${Y}x${Z}\", start=2.0)' -o l_{shape}.{format}"
...
test_prex "cubist -e 'cb.random_cube(\"${X}x${Y}x${H}x${Z}\")' -o og_{shape}.{format}"
...
test_prex "cubist -e 'cb.const_blocks_cube(\"10x20x30\")' -o cp_{shape}.{format}"
check_file_exists_and_has_size cp_${X}x${Y}x${Z}.raw $(( $XYZ * ${bytes_float32} ))

## --- 2D ---
typeset -i _x=8
typeset -i _y=10
test_prex "cubist -e 'cb.linear_cube(\"$_x x $_y\")' -o l1_{shape}.{format}"
check_file_exists_and_has_size l1_${_x}x${_y}.raw $(( $_x * $_y * ${bytes_float32} ))

test_prex "cubist -e 'cb.linear_cube(\"$_x x $_y\", start=1.0, increment=2.0)' -o l2_{shape}.{format}"
check_file_exists_and_has_size l2_${_x}x${_y}.raw $(( $_x * $_y * ${bytes_float32} ))

test_prex "cubist -i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1)' -P > res.out"
check_file_exists_and_has_content res.out 0

test_prex "cubist -i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1, tolerance=1.0)' -P > res.out"
check_file_exists_and_has_content res.out 1

test_prex "cubist -i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1, tolerance=10.0)' -P > res.out"
check_file_exists_and_has_content res.out 10

test_prex "cubist -i l1_{shape}.{format} -i l2_{shape}.{format} -s 8x10 -e 'cb.equals_num(i0, i1, tolerance=1e3)' -P > res.out"
